...
3. `.ai/features/[기능명]/01_dev.md`를 읽고 구현 요약, 변경 파일, 테스트 결과를 파악한다.
4. `.ai/features/[기능명]/02_review.md`를 읽고 리뷰 지적 사항을 파악한다.
5. `.ai/features/[기능명]/03_fix.md`를 읽고 수용/거부/보류 판단과 실제 수정 내용을 파악한다.
...
10. 검증 결과를 `.ai/features/[기능명]/04_verify.md`에 작성한다.
11. 검증 시작 시점의 현재 `HEAD`를 `verify_target_commit`으로 기록한다.
12. 모델 기준 판정이 `PASS`이면 `04_verify.md`와 검증 관련 변경을 하네스가 세 번째 커밋에 포함할 수 있게 워킹트리를 커밋 가능한 상태로 남긴다. 추천 커밋 메시지는 `[기능명][YYYYMMDD-hhmmss][04_verify]` 포맷을 사용한다.
