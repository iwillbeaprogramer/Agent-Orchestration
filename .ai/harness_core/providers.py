...
def raw_provider_command(ctx: dict[str, Any], provider: str) -> list[str]:
    ...
    if not command:
        raise ctx["HarnessError"](f"No provider command configured for {provider}")
...
def normalize_performance(ctx: dict[str, Any], value: Any | None) -> str:
    ...
    if performance not in ctx["PERFORMANCE_PROFILES"]:
        ...
        raise ctx["HarnessError"](f"Unknown performance profile {value!r}. Expected one of: {allowed}.")
    return performance
...
def preset_provider_for_stage(ctx: dict[str, Any], stage: str) -> str | None:
    ...
    if not provider:
        raise ctx["HarnessError"](f"No provider mapped for preferred_model={preferred!r} stage={stage}")
    return provider
...
def model_policy(ctx: dict[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(raw, dict):
        raise ctx["HarnessError"]("model_policy config must be an object.")
...
def pipeline_extracts_pc_candidates(ctx: dict[str, Any], pipeline_mode: Any) -> bool:
    return str(pipeline_mode or ctx["PIPELINE_MODE"]) in {"standard", "full"}
...
def provider_for_stage(ctx: dict[str, Any], stage: str, state: dict[str, Any] | None = None) -> str:
    ...
    candidates = ctx["candidate_providers_for_stage"](stage, policy, available)
    if not candidates:
        raise ctx["HarnessError"](f"No provider available for stage {stage}.")
    return candidates[0]
