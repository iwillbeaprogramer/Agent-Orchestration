...
- performance: medium
- output_file: .ai/features/project-initialize/02_dev.md
- result_json_file: .ai/features/project-initialize/02_dev.result.json
...
3. Write the human-readable stage output file exactly at `.ai/features/project-initialize/02_dev.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/project-initialize/02_dev.result.json`.
...
The harness reads `.ai/features/project-initialize/02_dev.result.json` first. Keep the `## 단계 결과` section in `.ai/features/project-initialize/02_dev.md` for humans, but write this JSON file for the harness.
...
## Current Git Hints
- current_head: 087a012aaf462c192297ca48fa4c656fdb9f968b
...
1. `.ai/features/project-initialize/00_spec.md`의 목표, 범위, 요구사항, 제외 항목, 위험도를 파악한다.
2. `.ai/features/project-initialize/01_plan.md`의 구현 접근 방식, 변경 파일 계획, 구현 단계, 위험 구간, 새 의존성, 테스트 전략, Git 기준점을 파악한다.
...
9. 구현이 끝나면 `.ai/features/project-initialize/02_dev.md`를 작성한다.
...
12. 하네스가 커밋할 수 있도록 워킹트리를 커밋 가능한 상태로 남기고, `harness_commit_required: true`와 추천 커밋 메시지를 기록한다. 추천 커밋 메시지는 `project-initialize[YYYYMMDD-hhmmss][02_develop]` 포맷을 사용한다.
...
계획을 그대로 수행할 수 없으면 `.ai/features/project-initialize/02_dev.md`에 아래 블록을 작성하고 멈춘다.
...
구현 완료 후 `.ai/features/project-initialize/02_dev.md`에 아래 형식으로 작성한다.
...
- .ai/features/project-initialize/02_dev.md (신규 / 수정)
...
- commit_message_suggestion: project-initialize[YYYYMMDD-hhmmss][02_develop]
- commit_scope:
  - .ai/features/project-initialize/02_dev.md
  - 구현 코드와 구현 테스트
