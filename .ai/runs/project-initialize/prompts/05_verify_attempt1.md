...
- performance: medium
- output_file: .ai/features/project-initialize/05_verify.md
- result_json_file: .ai/features/project-initialize/05_verify.result.json
- run_state: .ai/runs/project-initialize/run.json
- generated_at: 2026-05-28T15:55:26
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/project-initialize/05_verify.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/project-initialize/05_verify.result.json`.
...
The harness reads `.ai/features/project-initialize/05_verify.result.json` first. Keep the `## 단계 결과` section in `.ai/features/project-initialize/05_verify.md` for humans, but write this JSON file for the harness.
...
## Current Git Hints
- current_head: 3b846a6070e7f9047c68584b39f1d1dae3004bf6
...
- 하네스 실행 결과는 `.ai/runs/project-initialize/verification/latest.json`에 저장된다.
...
5. `.ai/features/project-initialize/03_review.md`를 읽고 리뷰 지적 사항을 파악한다.
6. `.ai/features/project-initialize/04_fix.md`를 읽고 수용/거부/보류 판단과 실제 수정 내용을 파악한다.
7. 의사결정 검증을 수행한다.
8. 동작 검증을 수행한다. 직접 실행한 명령은 기록하되, 최종 자동화 판정은 하네스가 실행하는 검증 명령 결과에 위임한다.
9. 검증 결과를 `.ai/features/project-initialize/05_verify.md`에 작성한다.
10. 검증 시작 시점의 현재 `HEAD`를 `verify_target_commit`으로 기록한다.
11. 모델 기준 판정이 `PASS`이면 `05_verify.md`와 검증 관련 변경을 하네스가 세 번째 커밋에 포함할 수 있게 워킹트리를 커밋 가능한 상태로 남긴다. 추천 커밋 메시지는 `project-initialize[YYYYMMDD-hhmmss][05_verify]` 포맷을 사용한다.
...
검증 완료 후 `.ai/features/project-initialize/05_verify.md`에 아래 형식으로 작성한다.
...
- verification_commit_message_suggestion: project-initialize[YYYYMMDD-hhmmss][05_verify]
