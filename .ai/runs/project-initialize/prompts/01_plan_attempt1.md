...
- performance: medium
- output_file: .ai/features/project-initialize/01_plan.md
- result_json_file: .ai/features/project-initialize/01_plan.result.json
- run_state: .ai/runs/project-initialize/run.json
- generated_at: 2026-05-28T15:32:04
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/project-initialize/01_plan.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/project-initialize/01_plan.result.json`.
...
The harness reads `.ai/features/project-initialize/01_plan.result.json` first. Keep the `## 단계 결과` section in `.ai/features/project-initialize/01_plan.md` for humans, but write this JSON file for the harness.
...
1. `.ai/features/project-initialize/00_spec.md`의 목표, 범위, 요구사항, 제외 항목, 위험도를 정확히 파악한다.
...
7. 결과를 `.ai/features/project-initialize/01_plan.md`에 작성한다.
...
계획 확정 후 `.ai/features/project-initialize/01_plan.md`에 아래 형식으로 작성한다.
