...
- generated_at: 2026-05-28T15:30:16
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/project-initialize/00_spec.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/project-initialize/00_spec.result.json`.
...
The harness reads `.ai/features/project-initialize/00_spec.result.json` first. Keep the `## 단계 결과` section in `.ai/features/project-initialize/00_spec.md` for humans, but write this JSON file for the harness.
...
8. `.ai/features/project-initialize/` 디렉토리가 없으면 생성한다.
9. 확정된 스펙을 `.ai/features/project-initialize/00_spec.md`에 아래 양식으로 작성한다.
...
질문이 필요하면 `.ai/features/project-initialize/00_spec.md`에 아래 블록만 작성하고 멈춘다.
...
스펙 확정 후 `.ai/features/project-initialize/00_spec.md`에 아래 형식으로 작성한다.
