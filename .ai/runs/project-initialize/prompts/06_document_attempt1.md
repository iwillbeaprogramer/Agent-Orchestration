...
- performance: medium
- output_file: .ai/features/project-initialize/06_document.md
- result_json_file: .ai/features/project-initialize/06_document.result.json
- run_state: .ai/runs/project-initialize/run.json
- generated_at: 2026-05-28T15:56:42
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/project-initialize/06_document.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/project-initialize/06_document.result.json`.
...
The harness reads `.ai/features/project-initialize/06_document.result.json` first. Keep the `## 단계 결과` section in `.ai/features/project-initialize/06_document.md` for humans, but write this JSON file for the harness.
...
난 주식 대시보드를 만들고싶어. 화면은 React 백엔드는 FastAPI로 구성해줘. 화면구성은 일단 미국의 대표지수/한국의 대표지수/ 기타국의 대표지수/ 환율 이 한번에 보였으면 좋겠어. 좀 보기좋게 나누긴 해야겠지. 
...
## Current Git Hints
- current_head: f9ef04c01249399189f6db9ad7cc11e9d37955a9
...
- 이 단계의 문서 산출물은 Git 커밋 대상이 아니라 artifact로 취급한다.
...
1. `.ai/features/project-initialize/05_verify.md`의 최종 판정이 PASS인지 확인한다.
2. 최종 판정이 PASS가 아니면 `.ai/features/project-initialize/06_document.md`에 `status: SKIPPED`를 기록하고 멈춘다.
3. `.ai/features/project-initialize/00_spec.md`의 목표, 범위, 요구사항을 파악한다.
4. `.ai/features/project-initialize/01_plan.md`를 읽고 구현 접근 방식, 검토된 대안, 위험 구간, 테스트 전략 등 설계 의사결정을 파악한다.
5. `.ai/features/project-initialize/02_dev.md`를 읽고 구현 내용을 파악한다.
6. `.ai/features/project-initialize/03_review.md`를 읽고 리뷰의 핵심 포인트를 파악한다.
7. `.ai/features/project-initialize/04_fix.md`를 읽고 최종 설계 결정과 수용/거부/보류 항목을 파악한다.
8. `.ai/features/project-initialize/05_verify.md`를 읽고 테스트 커버리지와 검증 결과를 파악한다.
9. `## 문서 생성 스크립트` 패턴을 참고하여 `.ai/runs/project-initialize/document_build.py`를 작성하고 실행하여 `.ai/docs/project-initialize_명세서.docx`를 생성한다.
10. 생성한 `.docx`가 유효한 Office Open XML 문서인지 아래 산출물 검증 절차로 확인한다.
11. 검증에 실패하면 가짜 `.docx`를 성공 산출물로 기록하지 말고 `.ai/features/project-initialize/06_document.md`에 `status: FAIL`과 구체적인 실패 사유를 기록한다.
12. 검증에 성공한 경우에만 생성 결과를 `.ai/features/project-initialize/06_document.md`에 기록한다.
...
- 문서 생성용 Python 스크립트는 `.ai/runs/project-initialize/document_build.py`에만 작성한다.
...
아래 패턴으로 `.ai/runs/project-initialize/document_build.py`를 작성하고 실행하여 `.docx`를 생성한다.
`project-initialize`, `[내용]` 부분을 실제 내용으로 채운다.
...
1. `.ai/docs/project-initialize_명세서.docx` 파일이 존재하는지 확인한다.
...
8. `[내용]`, `project-initialize`, `src/path/to/file.py`, `tests/test_xxx.py`, `패키지명` 같은 템플릿 placeholder가 남아 있으면 실패로 처리한다.
9. 위 검증 중 하나라도 실패하면 `status: FAIL`로 기록하고, `blocking_reason`에 실패한 검증 항목을 구체적으로 쓴다.

PowerShell 예시:

```powershell
Format-Hex -Path .ai\docs\project-initialize_명세서.docx | Select-Object -First 1
```
...
path = ".ai/docs/project-initialize_명세서.docx"
...
with ZipFile(path) as zf:
    ...
    forbidden_placeholders = [
        "[내용]", "project-initialize", "src/path/to/file.py", "tests/test_xxx.py", "패키지명"
    ]
...
문서 생성 후 `.ai/features/project-initialize/06_document.md`에 아래 형식으로 작성한다.
...
- 문서 생성용 임시 스크립트를 `.ai/runs/project-initialize/document_build.py` 외 경로에 작성하지 않는다.
