...
- performance: medium
- output_file: .ai/features/project-initialize/04_fix.md
- result_json_file: .ai/features/project-initialize/04_fix.result.json
- run_state: .ai/runs/project-initialize/run.json
- generated_at: 2026-05-28T15:50:28
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/project-initialize/04_fix.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/project-initialize/04_fix.result.json`.
...
The harness reads `.ai/features/project-initialize/04_fix.result.json` first. Keep the `## 단계 결과` section in `.ai/features/project-initialize/04_fix.md` for humans, but write this JSON file for the harness.
...
## Current Git Hints
- current_head: a453cf72dee482542a4b004760eea8383bfb2369
- changed_paths_excluding_runs: [".ai/features/project-initialize/03_review.md", ".ai/features/project-initialize/03_review.result.json"]
...
optional_inputs:
  ...
  - ".ai/runs/project-initialize/verification/latest.json"
outputs:
...
commit_policy: "always_commit_stage_04"
...
- 5단계가 하네스 검증 명령 실패로 되돌아온 경우 `.ai/runs/project-initialize/verification/latest.json`을 반드시 읽고, 실패한 명령의 stdout/stderr 경로와 returncode를 수정 근거로 사용한다.
...
4. `.ai/features/project-initialize/02_dev.md`를 읽고 원래 구현 의도와 Git 정보를 확인한다.
5. `.ai/features/project-initialize/03_review.md`를 읽고 리뷰 내용을 확인한다.
6. `.ai/features/project-initialize/05_verify.md`가 존재하고 최종 판정이 `FAIL`이면, 실패 항목과 `fix_inputs`를 03_review 이후의 추가 지적 사항으로 취급하여 우선 처리한다.
7. `.ai/runs/project-initialize/verification/latest.json`이 존재하면 하네스 검증 결과를 읽는다. `status: FAIL`이거나 `failed_commands`가 비어 있지 않으면 `05_verify.md`의 모델 판정보다 하네스 검증 결과를 우선한다.
...
13. 결과를 `.ai/features/project-initialize/04_fix.md`에 작성한다.
14. 최초 04_fix 실행이면 코드 변경이 없더라도 리뷰 처리 기록과 `04_fix.md`를 포함하여 하네스가 두 번째 커밋을 만들 수 있게 워킹트리를 커밋 가능한 상태로 남긴다. 추천 커밋 메시지는 `project-initialize[YYYYMMDD-hhmmss][04_fix]` 포맷을 사용한다.
...
- 이 단계는 기능당 고정 커밋 3개 중 두 번째 커밋을 하네스가 만들 수 있게 준비하는 단계이다.
- 05_verify 실패 후 재진입한 경우에도 모델은 직접 amend하지 않고, 하네스가 기존 두 번째 커밋을 갱신할 수 있게 변경을 남긴다.
...
개선 완료 후 `.ai/features/project-initialize/04_fix.md`에 아래 형식으로 작성한다.
...
- commit_message_suggestion: project-initialize[YYYYMMDD-hhmmss][04_fix]
