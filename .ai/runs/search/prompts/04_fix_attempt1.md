...
- performance: medium
- output_file: .ai/features/search/04_fix.md
- result_json_file: .ai/features/search/04_fix.result.json
- run_state: .ai/runs/search/run.json
- generated_at: 2026-05-29T09:48:47
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/search/04_fix.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/search/04_fix.result.json`.
...
The harness reads `.ai/features/search/04_fix.result.json` first. Keep the `## 단계 결과` section in `.ai/features/search/04_fix.md` for humans, but write this JSON file for the harness.
...
## Current Git Hints
- current_head: 68a86c7f146ade5d2989b477cfd39f452eb1cf47
- changed_paths_excluding_runs: [".ai/features/search/03_review.md", ".ai/features/search/03_review.result.json"]
...
optional_inputs:
  ...
  - ".ai/runs/search/verification/latest.json"
outputs:
...
- 5단계가 하네스 검증 명령 실패로 되돌아온 경우 `.ai/runs/search/verification/latest.json`을 반드시 읽고, 실패한 명령의 stdout/stderr 경로와 returncode를 수정 근거로 사용한다.
...
2. `.ai/features/search/00_spec.md`의 목표, 범위, 요구사항, 위험도를 파악한다.
3. `.ai/features/search/01_plan.md`의 구현 접근 방식, 변경 파일 계획, 위험 구간, 의존성, 테스트 전략을 파악한다.
4. `.ai/features/search/02_dev.md`를 읽고 원래 구현 의도와 Git 정보를 확인한다.
5. `.ai/features/search/03_review.md`를 읽고 리뷰 내용을 확인한다.
6. `.ai/features/search/05_verify.md`가 존재하고 최종 판정이 `FAIL`이면, 실패 항목과 `fix_inputs`를 03_review 이후의 추가 지적 사항으로 취급하여 우선 처리한다.
7. `.ai/runs/search/verification/latest.json`이 존재하면 하네스 검증 결과를 읽는다. `status: FAIL`이거나 `failed_commands`가 비어 있지 않으면 `05_verify.md`의 모델 판정보다 하네스 검증 결과를 우선한다.
...
13. 결과를 `.ai/features/search/04_fix.md`에 작성한다.
14. 최초 04_fix 실행이면 코드 변경이 없더라도 리뷰 처리 기록과 `04_fix.md`를 포함하여 하네스가 두 번째 커밋을 만들 수 있게 워킹트리를 커밋 가능한 상태로 남긴다. 추천 커밋 메시지는 `search[YYYYMMDD-hhmmss][04_fix]` 포맷을 사용한다.
...
개선 완료 후 `.ai/features/search/04_fix.md`에 아래 형식으로 작성한다.
...
- commit_message_suggestion: search[YYYYMMDD-hhmmss][04_fix]
