...
- performance: medium
- output_file: .ai/features/debug/01_plan.md
- result_json_file: .ai/features/debug/01_plan.result.json
- run_state: .ai/runs/debug/run.json
- generated_at: 2026-05-28T16:20:12
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/debug/01_plan.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/debug/01_plan.result.json`.
...
The harness reads `.ai/features/debug/01_plan.result.json` first. Keep the `## 단계 결과` section in `.ai/features/debug/01_plan.md` for humans, but write this JSON file for the harness.
...
1. `.ai/features/debug/00_spec.md`의 목표, 범위, 요구사항, 제외 항목, 위험도를 정확히 파악한다.
...
7. 결과를 `.ai/features/debug/01_plan.md`에 작성한다.
...
계획 확정 후 `.ai/features/debug/01_plan.md`에 아래 형식으로 작성한다.
