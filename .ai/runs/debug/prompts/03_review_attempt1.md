...
- performance: medium
- output_file: .ai/features/debug/03_review.md
- result_json_file: .ai/features/debug/03_review.result.json
- run_state: .ai/runs/debug/run.json
- generated_at: 2026-05-28T16:29:40
- defaults_mode: true
...
3. Write the human-readable stage output file exactly at `.ai/features/debug/03_review.md`.
4. Also write the machine-readable stage result JSON exactly at `.ai/features/debug/03_review.result.json`.
...
The harness reads `.ai/features/debug/03_review.result.json` first. Keep the `## 단계 결과` section in `.ai/features/debug/03_review.md` for humans, but write this JSON file for the harness.
...
8. 리뷰 결과를 `.ai/features/debug/03_review.md`에 작성한다.
...
리뷰 완료 후 `.ai/features/debug/03_review.md`에 아래 형식으로 작성한다.
